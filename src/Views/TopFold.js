...
import AfterClickingYT from "../Components/AfterClickingYT"; // Import the popup component
import "./TopFold.css";

const TopFold = () => {
  const [showPopup, setShowPopup] = useState(false); // State for controlling the popup visibility
  const [blurred, setBlurred] = useState(false); // State for controlling the blur effect

  // Function to handle the YouTube logo click
  const handleYouTubeClick = () => {
    setBlurred(true); // Apply blur effect
    setShowPopup(true); // Show the popup when the YouTube logo is clicked
  };

  return (
    <div className="topfold">
      {/* Wrapper for the background content */}
      <div className={`background-content ${blurred ? 'blurred' : ''}`}>
        ...
        <div className="logo-grid-section">
          <h3 className="logo-grid-title">Host a Watch Board on:</h3>
          <div className="logo-grid">
            <img 
              src={YouTube} 
              alt="YouTube" 
              className="logo-grid-item youtube" 
              onClick={handleYouTubeClick} // Click event for triggering the popup
            />
      ...
      {/* Popup Component (After Clicking YouTube) */}
      {showPopup && <AfterClickingYT />}
    </div>
  );
};
